...
pub fn extract_secret_keys(redis_addr: &str) -> Vec<String> {
    let secret_keys = redis_keys(redis_addr, "origin_secret_key").expect("error extracting secret keys");
    redis::from_redis_value::<Vec<String>>(&secret_keys).unwrap()
}
