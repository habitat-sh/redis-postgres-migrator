...
fn test_migrate() {
    ...
    let origin = redis_lib::create_origin(TEST_REDIS_ADDR,
                                              "core",
                                              5000;
    ...
    let package1 = redis_lib::create_package(TEST_REDIS_ADDR, hart_file("core-libossp-uuid-1.6.2-20161214075157-x86_64-linux.hart"));
    let package2 = redis_lib::create_package(TEST_REDIS_ADDR, hart_file("core-libcap-2.24-20161208223353-x86_64-linux.hart"));

    migrators::origin::PackageMigrator::new(TEST_REDIS_ADDR.to_string(), ds.clone()).migrate();

    let pg_package1 = postgers_lib::get_package_by_ident(ds.clone(), "core/libossp-uuid/1.6.2/20161214075157").expect("could not get libossp");
    let pg_package2 = postgers_lib::get_package_by_ident(ds.clone(), "core/libcap/2.24/20161208223353").expect("could not get libcap");

    assert_eq!(package1.get_ident(), pg_package1.get_ident());
    assert_eq!(package1.get_checksum(), pg_package1.get_checksum());
    assert_eq!(package1.get_manifest(), pg_package1.get_manifest());
    assert_eq!(package1.get_deps(), pg_package1.get_deps());
    assert_eq!(package1.get_tdeps(), pg_package1.get_tdeps());
    ...
    assert_eq!(package2.get_ident(), pg_package2.get_ident());
    assert_eq!(package2.get_checksum(), pg_package2.get_checksum());
    assert_eq!(package2.get_manifest(), pg_package2.get_manifest());
    assert_eq!(package2.get_deps(), pg_package2.get_deps());
    assert_eq!(package2.get_tdeps(), pg_package2.get_tdeps());
